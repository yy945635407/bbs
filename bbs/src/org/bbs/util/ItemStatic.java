...
public class ItemStatic {
	private static int maxUserNum = 10000;
	private static int maxPostNum = 10000;
	private static int userNum = 0;
	private static int postNum = 0;
	private static int targetUserId = 0; //目标用户ID
	private static int simItemNum = 0; //选择的相似用户的个数
	private static int[][] allPosttoUser= new int[maxPostNum][maxUserNum]; //所有帖子的用户访问数据
	private static List<List<Object>> allPostSimilarities = null;
	private static List<Integer> targetUserHasViewed = null;
	private static List<List<Object>> recommendPosts = null;
	
	public ItemStatic() {
	}
	
	public static void Initial(int userNum1, int postNum1,int target, int simiNum) {//初始化构造函数
		//params: userNum:总用户个数 postNum:总帖子个数 target:目标用户ID simNum:选择的相似用户个数 
		long beforeQuery = System.currentTimeMillis();
		userNum = userNum1;
		postNum = postNum1;
		targetUserId = target;
		simItemNum = simiNum>userNum?userNum:simiNum;
...
//		}
	...
	private static double calSimilarity(int[] post1, int[] post2){//计算两个帖子特征向量的余弦距离
...
//		System.out.println(up+"--"+down1+"--"+down2);
	...
	private static void calAllSimilarities(){//计算所有帖子与目标用户浏览过的帖子的相似度
...
//		}
	...
	private static void calRecommend(){//计算推荐的帖子(同时除去目标用户已经有记录的帖子)
...
//		}
	...
	private static void sortListOfList(List<List<Object>> list,int order){//二维列表排序函数
	...
	public static int[] recommend(int userNum, int postNum,int target, int simiNum, int recommendNum){//推荐recommendNum个帖子(ID)
		//冷启动处理
		long beforeInitial = System.currentTimeMillis();
		Initial(userNum, postNum, target, simiNum);
		long afterInitial = System.currentTimeMillis();
		System.out.println("initial:" + (afterInitial-beforeInitial) + "ms");
		IHistoryDao historyDao = new HistoryDaoImpl();
...
//		}
	...
	public static int[] randomRecommend(int num){
	...
	public String toString() {
		String string = userNum + "--" + postNum + "--" + targetUserId + "--" + simItemNum;
		return string;
	}
}
