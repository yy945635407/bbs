...
<style>
    ...
    #post{
      ...
      margin-bottom: 15px;
      padding: 30px;
      background-color: rgba(104, 24, 223, 0.15);
...
function search(dir){
	...
	//alert(currentPage);
	$.ajax({
		url:"QueryAllUnreviewedPostServlet",
		...
		success:function(result){
			...
			if(result.info=="yes"){//展示查询结果
				...
				$.each(posts, function(i, post){
					htm += "<div class='row'><div id='post' class='col-md-8'><a href='QueryPostByID?postID=" +
							post.postID + "'>" + post.title + "</a></div>"+"<div class='col-md-4'><a class='btn btn-md btn-primary btn-block' role='button' href='ReviewPostServlet?postID="+ post.postID +
							"'>审核</a></div>" + 
							"</div>";
			...
			if(page.totalPage<=1){//只有一页
				pageHelp += "<div class='col-md'></div>"+"<div class='col-md-2'></div>" +
